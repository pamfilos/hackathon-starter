...
var protelFileDir = './protelFiles/';
var filePrefix = 'airotel_avail_';
var results;
var protelFile;
var currentFile;
...
async.series([
  ...
  getMostRecentprotelFileName,
  // writeCurrentRoomAvailabilityGroup_calculated,
  getAvailabilityCal,
  cronTasks
]);

function cronTasks(next){
  new CronJob('0 0-59/30 * * * *', runEvery30, null, true, 'Europe/Athens');
}

function getMostRecentprotelFileName(next) {
    var files = fs.readdirSync(protelFileDir);
    console.log(files);
    // use underscore for max()
    var filename =  _.max(files, function (f) {
        var fullpath = path.join(protelFileDir, f);
    ...
    protelFile = protelFileDir+'/'+filename;
    next();
}
...
function runEvery30(){
  async.series([
    function(next){
      ...
      protelFile = currentFile = protelFileDir+filePrefix+moment().format('DDMMYYYY_HHmmss');
      next();
    },
    writeCurrentRoomAvailabilityGroup_calculated,
    getAvailabilityCal
...
function getHotelInfoFromConfig(next){
  getObjectFromFile(hotelConfigFile, function(err, object){
    console.log('Reading Configuration file [',hotelConfigFile,']');
    if (object['hotels']){
      hotels = object['hotels'];
      hotel_ids = _.pluck(hotels, 'id');
      hotel_names = getHotelNames();
      console.log('HOTELS:',hotel_names);
    }
    next();
  });
}
...
function getAvailabilityCal(next){
  getObjectFromFile(protelFile, function(err, object){
    ...
    console.log('HOTEL AVAILABILITY RECONSTRUCT');
...
function getPriceCalculationsGroup(month, callback){
  ...
  async.each(
    ...
    function(error){
      ...
      temp_results['current_file'] = protelFile;
...
function getEventsCurrentMonth(callback){
  var date = moment();
  getEventsMonth(date, callback);  
}

function getEventsMonth(month, callback) {
  if(!moment.isMoment(month)) month = moment(month, 'YYYY-MM-DDTHH:mm:ss+03:00');
  
  var monthStart = moment(month).startOf('month').valueOf();
  var monthEnd = moment(month).endOf('month').valueOf();

  console.log('Start:',monthStart,'-- End:',monthEnd);

  Event.find( 
    { $or : [ 
        { start : {$gt: monthStart, $lt: monthEnd} },
        { end :  {$gt: monthStart, $lt: monthEnd} } 
      ]
    }, 
    function(err, events){
      if (err) return null;
...
function getEventsAgendaCurrentMonth(callback){
  var date = moment();
  getEventsAgendaMonth(date, callback);  
}

function getEventsAgendaMonth(month, callback){
  if(!moment.isMoment(month)) month = moment(month, 'YYYY-MM-DDTHH:mm:ss+03:00');
  ...
  getEventsMonth(month, function(err, events){
    if (! events) callback("No events found", null);

    _.each(events, function(event){
      eventRange = moment.range(event.start, event.end);
      tmpRange = monthRange.intersect(eventRange);

      tmpRange.by('day', function(day){
        daystring = day.format('DDMMYYYY');
        if (agenda[daystring] == undefined) agenda[daystring] = [];
        agenda[daystring].push(event._id);
      });
    });

    agenda = { agenda: agenda, events: events };
    callback(err, agenda);
  });
}
...
exports.getHotelNames = getHotelNames;
...
exports.getEventsCurrentMonth = getEventsCurrentMonth;
exports.getEventsMonth = getEventsMonth;
exports.getEventsAgendaMonth = getEventsAgendaMonth;
exports.getEventsAgendaCurrentMonth = getEventsAgendaCurrentMonth;
